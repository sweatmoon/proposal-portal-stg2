...
app.get('/ppt-generate', (c) => {
  ...
  <div class="p-6 md:p-8 max-w-5xl" id="pptGenRoot">
    <h1 class="text-2xl font-bold text-slate-800 flex items-center gap-2 mb-1">
      <i class="fas fa-file-powerpoint text-indigo-500"></i> 첨부 생성
    </h1>
    <p class="text-sm text-slate-500 mb-6">
      DB에 적재된 사업 목록입니다. 원하는 사업의 "첨부PPT 생성" 버튼을 눌러 첨부 항목을
      선택하고 템플릿을 업로드하면, 해당 사업 데이터로 채워진 pptx 파일이 바로 다운로드됩니다.
    </p>
  ...
  <!-- ▲ [ppt-portal 추가 기능] 끝 -->
  `
  return c.html(layout('첨부 생성', body, 'ppt-generate'))
})
