...
import { fetchLocalTaxCertificatePdf, fetchCompanyStampPng, fetchLatestPptxOrPdfFromFolder, type CompanyStampType } from '../lib/nas-client.js'
...
export async function buildLocalTaxCertificateZip(
  ...
  const [sourceFile, stampPng] = await Promise.all([
    override?.nasPath ? fetchLatestPptxOrPdfFromFolder(override.nasPath, label) : fetchLocalTaxCertificatePdf().then(buf => buf ? { buf, isPdf: true } : null),
